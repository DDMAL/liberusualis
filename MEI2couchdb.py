    ...
    if meifile.get_system(seq[0]) != meifile.get_system(seq[len(seq)-1]): #then the sequence spans two systems and we must store two seperate locations to highlight
        for i in range(1,len(seq)):
            ...
            if meifile.get_system(seq[i-1]) != meifile.get_system(seq[i]): 
                ...
                lrx1 =  int(findbyID(zones, seq[0].parent.parent.facs).lrx)
                lry1 =  int(findbyID(zones, seq[0].parent.parent.facs).lry)
                ulx2 =  int(findbyID(zones, seq[0].parent.parent.facs).ulx)
                uly2 =  int(findbyID(zones, seq[0].parent.parent.facs).uly)
                lrx2 =  int(findbyID(zones, seq[0].parent.parent.facs).lrx)
                lry2 =  int(findbyID(zones, seq[0].parent.parent.facs).lry)
                return [{"ulx": int(ulx1) ,"uly": int(uly1), "height": abs(int(uly1) - int(lry1)), "width": abs(int(ulx1) - int(lrx1))},{"ulx": int(ulx2) ,"uly": int(uly2), "height": abs(int(uly2) - int(lry2)), "width": abs(int(ulx2) - int(lrx2))}] 
        ...
        lrx =  int(findbyID(zones, seq[len(seq)-1].parent.parent.facs).lrx)
        lry =  int(findbyID(zones, seq[len(seq)-1].parent.parent.facs).lry)
        return [{"ulx": int(ulx) ,"uly": int(uly), "height": abs(int(uly) - int(lry)), "width": abs(int(ulx) - int(lrx))}]
...
couch = couchdb.Server("http://132.206.14.43:5984") #couchdb.Server() should work too, but once it didn't so I hardcoded the address
