...
def getPitchNames(seq):
    ...
    midipitch = []
    for note in seq:
        pnames.append(note.pitch[0]) # a string of pitch names e.g. 'gbd'
        midipitch.append(int(convertStepToPs(str(note.pitch[0]), int(note.octave))))
...
if __name__ == '__main__':
    ...
    for ffile in meifiles:
        processMeiFile(ffile, longest_gram, shortest_gram)
        solrconn.commit()
        systemcache.clear()
        idcache.clear()
